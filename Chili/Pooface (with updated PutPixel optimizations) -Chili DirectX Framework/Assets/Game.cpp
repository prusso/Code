	...
	
{
	...
	{
		poo[index].x = rand() % (800 - 24);
		poo[index].y = rand() % (600 - 24);
...
void Game::UpdateFace()
{
	...
	// keyboard controls to move the face
...
void Game::UpdatePoo()
{
		for( int index = 0; index < nPoo; index++)
		{
			if (!pooIsEaten[index])
			{
				if (faceX + 20 > poo[index].x && faceX < poo[index].x + 24
					&& faceY + 20 > poo[index].y && faceY < poo[index].y + 24)
				{
...
{
	// example of a filled-in circle
	gfx.DrawDisc(400,300,5,254,221,88);
	...
	gfx.DrawDisc(mid.x, mid.y, 5, 0, 255, 255);

	for (int index = 0; index < nPoo; index++)
	{
		if ( !pooIsEaten[index])
		{
			DrawPoo( poo[index].x, poo[index].y);
		}
	}
	// draw the face
...
// this is an example of a global function defined after it is being used
// but since it is "forward declared" in the Game.h header file, it is useable!
