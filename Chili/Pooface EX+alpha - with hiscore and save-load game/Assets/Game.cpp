...
{
	for (int index = 0; index < NSCORES; index++ )
	{
...
{
	int index;
	for (index = 0; index < NSCORES && scores[index].score >= score.score; index++);

	if (index < NSCORES )
	{
		for (int shuffleIndex = NSCORES - 2; shuffleIndex >= index; shuffleIndex--)
		{
...
void Game::SaveScore()
{
	...
	for (int index = 0; index < NSCORES && scores[index].score >= 0; index++)
	{
...
void Game::LoadScore()
{
	...
	if ( pFile )
	{
		...
		do 
		{
			...
			index++;
		} while (!fseek(pFile,5,SEEK_CUR) && index < NSCORES);
		fclose(pFile);
	}
}
